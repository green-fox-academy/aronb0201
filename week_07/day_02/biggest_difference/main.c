...
float average_of_all_exam(struct class* clases);
int main() {
    int class_number;
    int counter = 0;
    float average;
    ...
    printf("average iz %f", average_of_all_exam(clases));
...
float average_of_all_exam(struct class* clases){
    float average;
    for (int i = 0; i < sizeof(clases)/ sizeof(*clases); ++i) {
        for (int j = 0; j <clases[i].student_number ; ++j) {
            average =+ clases[i].students[i].exams;

        }
        return (average/clases[i].student_number);
